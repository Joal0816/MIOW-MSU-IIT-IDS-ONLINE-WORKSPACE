...
export const schemas = {
  ...
  profileInput: z.object({
    ...
    grade_level: z.number().int().min(7).max(12).nullable().optional(),
  ...
  profilePatch: z.object({
    ...
    patch: z
      .object({
        ...
        grade_level: z.number().int().min(7).max(12).nullable(),
  ...
  courseInput: z.object({
    ...
    grade_level: z.number().int().min(7).max(12),
...
export async function verifyPinLogin(login: string, secret: string) {
  ...
  // Separate parameterized .eq() lookups per identifier column — the raw
  // input is never interpolated into a PostgREST filter expression (filter
  // injection), and the secret is never part of the query; it is verified
  // against the stored bcrypt hashes instead.
  let p = await unwrap<any>(
    db.from("profiles").select("*").eq("email", identifier.toLowerCase()).is("deleted_at", null).maybeSingle(),
  );
  if (!p) {
    p = await unwrap<any>(
      db.from("profiles").select("*").eq("student_id", identifier).is("deleted_at", null).maybeSingle(),
    );
  }
  if (!p) {
    p = await unwrap<any>(
      db.from("profiles").select("*").eq("username", identifier.toLowerCase()).is("deleted_at", null).maybeSingle(),
    );
  }
...
export async function deleteUser(adminId: string, id: string) {
  ...
  await unwrap(
    db
      .from("profiles")
      .update({ deleted_at: new Date().toISOString(), rfid_uid: null, pin: null, pin_hash: null })
      .eq("id", id),
  );
...
async function assertUniqueIdentity(
  fields: { email?: string | null; employee_id?: string | null; rfid_uid?: string | null },
  exceptId?: string,
) {
