...
export const Route = createFileRoute("/dashboard/admin/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Campus overview: students, courses, attendance and announcements." },
      { property: "og:title", content: "Admin Dashboard | MIOW - MSU-IIT IDS Online Workspace" },
      { property: "og:description", content: "Campus overview: students, courses, attendance and announcements." },
    ],
  }),
...
function AdminDashboard() {
  ...
  const { data: courseCount } = useQuery({ queryKey: ["count", "courses"], queryFn: () => countRows("courses"), enabled: !!profile });
  const { data: announcementCount } = useQuery({ queryKey: ["count", "announcements"], queryFn: () => countRows("announcements"), enabled: !!profile });
  const { data: logs } = useQuery({ queryKey: ["attendance-all"], queryFn: () => listAllAttendance(50), enabled: !!profile });
    ...
    <AppShell nav={ADMIN_NAV} profile={profile} subtitle="Admin Console">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="p-5">
          ...
          <Link to="/dashboard/admin/students" className="mt-1 flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
        ...
        <Card className="p-5">
          ...
          <Link to="/dashboard/admin/courses" className="mt-1 flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
            Manage <ArrowRight className="h-3 w-3" />
          </Link>
        </Card>
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Tapped In Today</p>
        ...
        <Card className="p-5">
          ...
          <Link to="/dashboard/admin/announcements" className="mt-1 flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
      ...
      <div className="mt-6">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link to="/dashboard/admin/settings" className="flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
            ...
            <div key={l.id} className="flex items-center gap-3 p-4">
              <div
                className={`flex h-9 w-9 items-center justify-center rounded-xl ${
                  l.scan_type === "in" ? "bg-emerald-100 text-emerald-600" : "bg-sky-100 text-sky-600"
                }`}
              >
                <CalendarCheck className="h-4 w-4" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold">{nameOf.get(l.student_id) ?? "Unknown student"}</p>
                <p className="text-xs text-muted-foreground">{fmtTime(l.timestamp)}</p>
              </div>
              <Badge tone={l.scan_type === "in" ? (l.status === "late" ? "amber" : "green") : "sky"}>
                {l.scan_type === "in" ? (l.status === "late" ? "IN · Late" : "IN · On time") : "OUT"}
              </Badge>
            </div>
          ))}
          {(logs ?? []).length === 0 && (
            <p className="p-6 text-center text-sm text-muted-foreground">No scans recorded yet today.</p>
          )}
        </Card>
      </div>
    </AppShell>
